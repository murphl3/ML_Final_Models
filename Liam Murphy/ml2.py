...
start = time.time()
tmp = pd.read_csv("Customer-Churn-Records.csv", index_col="RowNumber").drop(["Surname", "CustomerId", "Complain"], axis=1)
...
lr = sklearn.linear_model.LogisticRegressionCV(Cs=[10 ** i for i in range(-5, 5)], solver="saga", penalty="elasticnet", cv=80, max_iter=1000, scoring="f1_micro", random_state=get_seed(), n_jobs=3, l1_ratios=[0.01 * i for i in range(101)], verbose=3)
...
file = open("no_complain_output.txt", "w")
file.write("Params:")
